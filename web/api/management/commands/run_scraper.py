...
from backend.scraper_module.pdf_spider import PdfSpider
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(csv_path):
            self.stderr.write("No CSV file generated by spider.")
            return

        df = pd.read_csv(csv_path)
        for _, row in df.iterrows():
            pdf_url = row['file_path']
            ...
            try:
                ...
                if pdf:
                    ...
                    if not pdf.origin_urls.filter(url=pdf_url).exists():
                        PDFOriginURL.objects.create(pdf_file=pdf, url=pdf_url)
                    ...
                    PDFOriginURL.objects.create(pdf_file=new_pdf, url=pdf_url)
                    self.stdout.write(f"Neues PDF gespeichert: {file_name}")

            except Exception as e:
                self.stderr.write(f"Fehler beim Download von {pdf_url}: {e}")
